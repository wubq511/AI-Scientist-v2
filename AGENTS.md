...
`ai_scientist/` contains the Python package. Ideation starts in `perform_ideation_temp_free.py` (`new-run` admits an Ideation Run; `resume` continues one suspended run by exact run_id; `validate`/`export` verify and sanitize a sealed Evidence Chain; `evaluation` manages post-seal qualitative Evaluation Artifacts; the pre-fork `legacy` baseline was contracted out of the entry by ticket 13, leaving a standard-library-only import closure pinned by `tests/test_ideation_import_contract.py`); the DeepSeek adapter boundary is `ideation/deepseek.py`, retained legacy providers stay in `llm.py` and legacy literature tools in `tools/` for retained downstream code only, and the run admission boundary is in `ideation/` (`run_store.py`, `pricing.py`, `retrieval.py`, `admission.py`, `resume.py`). The execution controller `ideation/controller.py` seals every admitted run with an explicit Terminal Outcome: `success` or `failed` via canonical `seal.json` (hygiene hits, the run-level retrieval backstop, and deterministic adapter/retriever/evidence failures seal `failed`; environment-class failures and SIGINT/SIGTERM leave the run unsealed as a Run Suspension). The resume path (`ideation/resume.py` + `rebuild_resume_plan` in the controller) re-verifies the full chain and admission pins, quarantines approved-window orphan artifacts, rebuilds control state from events/artifacts under a new writer epoch, requires a fresh write-once cost approval for the remaining-work bound, and continues to the terminal seal. `data/raw/` is the ignored interview dataset. `artifacts/ideation-runs/<run_id>/` is the private, gitignored raw Evidence Chain root for admitted runs. `.agents/skills/` defines workflows; `work-logs/` is shared memory across agents and sessions. Treat `docs/task/` as private, ignored background.
