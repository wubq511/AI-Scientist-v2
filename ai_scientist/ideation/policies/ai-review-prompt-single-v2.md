...
- 材料包含一条 `audit_statement`（审计范围声明），逐项列明已完成检查的范围与
  `complete`/`incomplete` 状态。凡涉及审计事实的判断必须引用它，并受其声明的范围
  约束。具体地：`contamination_signal` 与 `leakage_review` 判为 `judged` 时，其
  `evidence_refs` 中必须至少有一条引用指向该 `audit_statement` 来源（缺此引用的
  评审会被程序整份拒绝）。
...
| contamination_signal | 已审查材料中 reference 难以解释的 target 独有命名或原文重合；judged 时必须引用 `audit_statement` 来源 | none_found 证明没有训练污染 |
...
可判 `none_found`，但 rationale 必须写明仅限本材料包范围，且至少一条引用指向
`audit_statement` 来源；若判断所需检查缺失则弃权。
