...
Once the prototype has answered its question, capture the answer and the prototype the way the [SKILL](SKILL.md) describes. Record the validated reducer / machine / function shape as part of the decision. Lift or reimplement it in the real module only when implementation is separately authorized; keep the HTML shell as the re-runnable primary source.

## Anti-patterns

- **Don't add a production test suite.** Add only the fixtures or checks needed to trust the state transitions being judged.
