...
Once a variant has won, capture the answer (which variant and why), then capture the prototype the way the [SKILL](SKILL.md) describes. In a planning workflow, stop after recording the decision and context pointer. If implementation is separately authorized, translate the winner into production-quality code and move the raw prototype set out of main:

- **Sub-shape A**: implement the approved design in the existing page; drop the losing variants and the switcher from main.
- **Sub-shape B**: implement the approved design as a real route; drop the throwaway route and the switcher from main.
