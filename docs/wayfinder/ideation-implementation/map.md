...
- [Seal explicit non-success outcomes](tickets/09-seal-explicit-non-success-outcomes.md)：以固定优先级 finalization gate（hygiene → structure → grounding → duplicate，每轮只报最高优先级）、版本化 payload hygiene 模式列表（命中即 terminal，不回灌不掩盖）、controller/provider/retriever_evidence 三类封闭 terminal 失败词汇与完整 artifact inventory，使 hygiene 命中、run 级 retrieval backstop 与确定性 adapter/retriever/evidence 失败都产生可解释的 terminal `failed` seal；suspend 类失败保持 unsealed（ticket 10 接管）。
