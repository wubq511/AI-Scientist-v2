...
- [x] result ingestion 在产生 pair packet 前验证 Run Specification/profile identity、case/input hashes、execution-code pin（`admission.code.commit` 等于 pinned clean HEAD）、Run Seal、Evidence Chain、sanitized export、Evaluation Artifact coverage、finish reason、attempt topology、latency 和 actual cost；不完整、corrupt、drifted 或 cross-case evidence fail closed。
...
- **Result ingestion**（`ingest_comparison_result`）：按序验证 matrix self-pin（`frozen_matrix_digest` 复核）→ `validate_evidence_chain(check_sealed=True)` → execution-code pin（`load_execution_code_pin` 读取 package 私有 pin，`admission.code.commit` 不等 → `EXECUTION_CODE_PIN_MISMATCH`，pin 缺失 → `EXECUTION_CODE_PIN_MISSING`）→ request/admission 的 profile pin/case_id/model 参数/budgets → frozen matrix slot 的 workshop/corpus hash pin → sanitized export 存在且 `prompt_profile.profile_id`/case_id 一致 → Evaluation Artifact coverage（复用 `list_evaluation_coverage`，要求 covered idea）→ 从 sealed event chain 派生 `RunMetrics`（terminal_outcome、finish_reasons、physical_attempt_count、Σ`provider_attempt.finished.duration_ms` latency、Σcost_cny，cost/duration 解析带 `ArithmeticError` guard）。`deterministic_validation_passed` 是本次 ingestion 实际运行的 gate（chain/seal/export/coverage/identity pins）的合取结果——fail closed 路径根本不产生 metrics；reducer 的零容忍门槛消费该标志。corrupt/drift/cross-case → `COMPARISON_IDENTITY_MISMATCH`/`INPUT_HASH_DRIFT`/`SANITIZED_EXPORT_MISSING`/`EVALUATION_ARTIFACT_MISSING` 等 fail closed。
...
- **测试**：`tests/test_prompt_comparison.py` 当前 72 个测试；除原 selection/matrix/blind/ingestion/verdict/reducer/E2E 覆盖外，新增 threshold crossing、exact/one-cent-over hard-cap reservation、ledger arithmetic/slot identity、guarded outer command、matrix/threshold/opening-balance drift、production inner parser、CLI 越序拒绝、write-once reservation 重复/并发拒绝，以及 execution-code pin 七场景（首 slot 创建与同 commit 安全重试、slot 1 后缺 pin、commit drift 与 stale pin 保留、exclusive-create 并发拒绝、dirty worktree 启动拒绝、ingestion admission-commit 不匹配、ingestion 缺 pin）。全程零网络、零真实 provider、零费用；不读活动 `.env`。
...
## Execution-code 单变量修正 (2026-09-04, post-close)

- Originating session 在预算修正后的双轴自审中确认：已批准的 one-major-variable 合同要求 Prompt Profile 是唯一执行控制差异，但 frozen matrix 没有执行代码 pin，slot runner 与 ingestion 都不校验 `admission.code.commit`；跨 commit 执行会把 runtime 变化混入因果归因。Robert 明确批准该有界架构修正后实施（commit `ac34ab7`）。
