...
Robert 已批准 comparison protocol，并授权从第一性原理冻结原未决项。v1.1 采用 Python 3.13.7 + mandatory CPU FP32 reference path、pinned `intfloat/e5-small-v2` dense challenger、12 cases / 24 queries、题名 + `publisher_abstract` relevance scope、延时盲复核、固定 normalization/resource/promotion/output-budget rules。
