...
后续调研确认：已批准 experiment matrix 完整执行，但现有 pointwise synthetic qrels 没有直接测量冻结 ideation consumer 的实际产物，不能据此提前结束研究。仓库现有 234 个 eligible cases，仅 12 个已用于本轮，仍可从未使用 universe 冻结 fresh topics。当前提案是在全新 12-case/24-query batch 上做 paired ideation-consumer evaluation，以跨 provider、双位置一致的 blind pairwise 判断和 deterministic citation/quote gates 为主证据，并把 union/full-corpus ceiling 作为诊断；Robert 不承担论文打分或 12 次盲选。该 future revision 尚未批准，也未产生 API 调用或费用。本 ticket 保持 open；批准新协议前不能写 Resolution、更新 map 或实现 production ranker。
