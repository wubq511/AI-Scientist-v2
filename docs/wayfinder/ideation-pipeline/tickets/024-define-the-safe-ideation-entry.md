...
> 后续修订（2026-09-04）：[Cross-domain Ideation Prompt qualification](../../../agents/cross-domain-ideation-prompt-spec.md) 与 wayfinder 票 01（Admit and replay one versioned Prompt Profile）批准并落地 **Prompt Profile** 这个新的封闭、版本化 run-control seam。`new-run` 参数面新增 `--prompt-profile <id>`，仅接受 `ml-baseline-v1` / `cross-domain-v1` 两个登记 id；自由 prompt 文本、路径、fragment、未知 id 一律 fail closed，且不开放任意 `reasoning_effort`、`max_tokens`、model、provider 或 endpoint 覆盖。模型可见 prompt 字节改为按 Run Admission 固定的 profile 渲染（fresh run、resume、validation、export 同一 profile），registry 以 hash pin 防漂移；legacy v1.0.0 request/admission 解释为 `ml-baseline-v1`。这是封闭 run 输入，不是第四个 dependency-injection seam：controller 的三个注入缝（adapter、retriever、event emitter/progress stream）不变。
