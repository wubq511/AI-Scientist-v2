...
3. **Environment smoke — Windows dense preflight complete; evidence-hardening replay pending**：Mac ambient/reference runtimes 均通过 26 项 harness tests；Windows fresh Python 3.13.7 venv 已从 hash-locked binary distributions 安装，pinned E5 artifacts 已校验。两次 Windows CPU FP32 diagnostic attempts 的 canonical payload hashes 完全一致，E5 cold/warm/build/RSS/model/environment 六项均通过 v1.0 complexity gates。Harness 已新增 frozen environment lock、direct gate verdict、conservative source-inclusive RRF resources 与 Python socket deny guard；仍需用包含这些改动的精确 commit 做 Windows fresh replay。Finalist-only Mac replay仍待 development promotion；不读取 holdout qrels。
...
剩余的是执行证据，不是继续拍脑袋选参数：先用 evidence-hardening commit 做 Windows fresh replay；准备 approved cases/queries/qrels；运行 development、按需做 Mac finalist replay与 blind holdout。Python socket deny guard 覆盖当前 transport surface，但不冒充 Windows Firewall 级隔离。任何实际 winner、可选 accelerator backend 或 production ranker 仍必须由这些结果决定；`Choose and calibrate local literature ranking` 因此保持 open。
