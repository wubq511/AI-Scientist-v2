...
2026-09-06 采用修订（Robert 批准，见 [002 裁决](comparison-002-promotion-criteria.md)）：VM-CONTRACT-024-04/06 当前要求 cross-domain-v1 为唯一执行 profile，baseline 新建与恢复均拒绝；024-05 的 baseline golden 及 COMPARE 系列只验证历史证据解释，不代表保留 baseline 执行能力。历史合成夹具仅在构造旧证据期间局部模拟退役前策略，随后立即恢复当前策略；真正运行测试不启用该夹具。
...
| VM-CONTRACT-024-04 | Prompt Profile 封闭身份:`new-run` 默认且仅接受 cross-domain-v1，baseline 以 `PROMPT_PROFILE_RETIRED` 拒绝（不创建 run、不询价、不调用 provider）;自由文本/路径/fragment/未知 id/legacy 文档注入 profile 字段一律 fail closed;registry hash pin 漂移即拒绝 | prompt 身份封闭且防漂移 | 024, cross-domain-ideation-prompt-spec | dev-time;全部 gate | pytest pass (`tests/test_prompt_profiles.py`) |
...
| VM-QUAL-04 | comparison 消费侧机检面(ticket 03,离线迁移 rehearsal):ingestion coverage 双版本分支——v1 人工 `evaluation_artifact_v1`(VM-QUAL-01 语义不变)与 v2 AI 双评审 `evaluation_artifact_v2_ai`(已注册评估协议);AI coverage 缺失/无效均不可 ingest(`EVALUATION_ARTIFACT_MISSING`/`EVALUATION_ARTIFACT_INVALID`,`unaggregated` 视为缺失),`complete_resolved`/`complete_unresolved` 均可 ingest,一 run 双通道共存 fail closed(`EVALUATION_CHANNEL_CONFLICT`);ComparisonVault write-once `ai-verdicts/` 通道(schema `comparison-ai-verdict-v1.0.0`,与 Robert 盲审 `verdicts/` 分离,揭盲后记录 `VERDICT_AFTER_REVEAL`,同 packet hash 绑定与人工通道一致);reducer 消费 AI 判定要求已注册协议 manifest,无 manifest 即 `EVALUATION_PROTOCOL_MISMATCH`,AI 与人工判定混用一 reduction fail closed(全矩阵同一评估协议);每臂质量底线取自双评审共识记录,`violated` 拒绝且 `unresolved`/`not_evaluated` 亦由 `ai_quality_floor_unresolved` 阻止晋升判断(manifest `unresolved_blocks_quality_pass=True`);无 AI 参数时 reduction 输出与修订前逐字节一致;评审费用台账 `evaluation-cost-ledger-v1.0.0`(write-once、hash-linked、append-only + 并发写检测)与生成 spend ledger 分离,合并只读报告 `evaluation-cost-merged-report-v1.0.0` 携带未授权支出披露;迁移辅助:冻结包六文件哈希核验(`verify_generation_package_compatibility`/`evaluation verify-migration-package --expected-sha256 NAME=SHA256`)、交接哈希清单(`migration_handoff_manifest`/`verify_handoff_manifest`)、台账新旧判定(`ledger_recency_comparison`:same_state/local_newer/remote_newer,字节同进度歧义 `MIGRATION_LEDGER_AMBIGUOUS` 交 Robert 裁决);协议注册/拒场景、迁移包往返+漂移、handoff 缺失、recency 歧义与跨矩阵、AI 判定通道 E2E(ingestion→record→reducer)、协议必需拒绝、人工通道 byte-stability 全部离线覆盖 | 修订后评估协议的消费边界被机检锁定:判分角色替换有显式授权门槛、未决不冒充通过、费用与预算不混合,迁移在两个隔离工作区间可核对、不双端计费 | ai-assisted-ideation-evaluation-spec §7, 002 proposal 修订案, ai-assisted-ideation-evaluation ticket 03 | post-seal;dev-time;迁移时 | pytest pass(tests/test_comparison_migration_rehearsal.py 17 项, tests/test_evaluation_costs.py 23 项,全量 883 passed) |
...
**2026-09-05 修订说明三（versioned addendum）**：新增 VM-QUAL-04,交付修订说明二延至 ticket 03 的 comparison 消费侧机检面（ticket 03;ingestion 双版本分支、vault `ai-verdicts/` AI 判定通道、reducer AI 消费新闸门、评审费用台账与迁移辅助,均为离线编码验收,详见 [ai-review-authoring-contract-v2.md](ai-review-authoring-contract-v2.md) 的「Comparison 接入」与 [ai-review-migration-runbook.md](ai-review-migration-runbook.md)）。VM-QUAL-01/02/03 语义不变:v1 人工覆盖分支仍按原口径核算,AI 评审记录不满足 v1 covered 门槛,单评审与成对评审机检面维持原行。真实六次基础 smoke 已于 2026-09-05 执行并通过（4/4 完成条件,证据 [ai-review-real-smoke-evidence.md](../research/ai-review-real-smoke-evidence.md)）,但真实迁移未执行、评估协议修订未获 Robert 显式批准,故 VM-QUAL-04 目前只证明软件契约（离线编码验收 883 passed）;正式启用需 Robert 显式批准修订 + 真实迁移执行（按 runbook）+ 逐 run/评审费用授权。
