...
资格验证使用现有冻结 12-case Canary 的确定性四例子集：Genetics & Molecular Biology、Health & Medicine、Social & Behavioral Sciences、Materials Science 各一例；同一 cluster 内以 canonical case hash 选择，不读取 Target contribution 或运行结果。每个 case 运行 baseline/challenger 一对，共 4 对、8 个串行 Ideation Runs；两臂固定同一 Workshop、Corpus、model、`reasoning_effort=high`、`max_tokens=32768`、generation/reflection budgets、retriever、rubric 与验证规则，唯一主变量是 Prompt Profile。执行顺序和 A/B 映射各自 2/2 平衡并在结果前冻结。全部 8 个 run 还必须在同一 clean Git HEAD 下执行：首个 slot reservation 以 exclusive-create `0600` 写入 package 私有的 `execution-code-pin.json`，之后每个 slot 启动与每次 result ingestion 都必须与该 pin 一致；commit drift、dirty worktree 或 pin 缺失一律 fail closed，stale pin 作为证据保留，更换 pinned commit 只能由 Robert 显式批准新 Design Epoch。

付费执行采用两层预算语义。`5.00 CNY` 是累计实际支出的 Plan Gate **重审批阈值**：某个已单独批准的 run 可以在结算后跨过阈值，但下一 slot 必须停止并重新取得 Robert 批准；它不是无法由事后账本保证的“硬子上限”。`30.00 CNY` 是唯一 Canary **硬上限**：每个 provider request 前，用当前 stage 实际支出加该 run 按官方高峰价格计算的 `7.08 CNY` 最坏上界做 reservation，只有 projected ceiling `<= 30.00 CNY` 才可进入生产 `new-run`。冻结命令必须经过同进程 slot runner，在 exec 前验证 matrix、ledger、顺序和 write-once slot reservation；重复/并发启动同一 slot fail closed。生产 admission 的费用确认默认保留逐 run 交互式 `yes`；Robert 的批内预授权（2026-09-05 批准的规格修订）是替代通道：比较包内 write-once `run-preauthorization.json`（schema `comparison-preauthorization-v1.0.0`，经 `COMPARISON_PREAUTHORIZATION` 环境变量指向）逐条记录 covered slot 的 (run_index, case_id, prompt_profile_id, worst_case_bound_cny)，admission 在第 8 步重算最坏上界并核对覆盖条目，命中时把授权出处（approver、approved_at、文档 sha256、`confirmed_with=batch_preauthorization`）逐字写进 Run Admission `cost.approval`；环境变量已设但文档缺失/畸形/未覆盖该 slot/ceiling 漂移一律 fail closed，绝不静默回退交互提示；未设环境变量的 run（含批外 run）仍走交互式 `yes`。该通道只改变批准的采集方式，不豁免 reservation、Plan Gate、硬上限或 ledger 记账。
...
54. As Robert, I want all eight runs pinned to one clean execution commit, so that runtime code drift cannot confound Prompt Profile attribution.
...
- **Comparison is one-major-variable.** Both arms use `reasoning_effort=high` and `max_tokens=32768`; Prompt Profile is the only differing Run Specification field. A machine check rejects any matrix with additional arm differences. Execution code identity is pinned out of band: the first slot reservation exclusive-creates a package-private `0600` `execution-code-pin.json` recording the current clean Git HEAD, every later slot fails closed unless the current clean HEAD equals the pin, and result ingestion compares each sealed run's `admission.code.commit` against the same pin before the run may enter the ledger, a pair packet or the reducer. A stale pin is never deleted silently; changing the pinned commit requires Robert's explicit new-epoch decision.
...
- **Live comparison commands are guarded and serial.** Every command carries the frozen matrix file SHA-256 and the `5.00 CNY` reapproval threshold. `scripts/run-prompt-comparison-slot` checks those external pins, the immutable `0.14 CNY` opening balance and all ledger arithmetic, requires `run_index = ingested_entries + 1`, recomputes `7.08 CNY` from the pinned price table, then writes an exclusive `0600` slot reservation whose reservation seam resolves the current clean Git HEAD (a dirty worktree fails as `DIRTY_WORKTREE`), creates or verifies the write-once execution-code pin and records the pinned commit, and immediately execs production `new-run`. A stale reservation or stale pin blocks retry until it is reconciled against the Evidence Chain; neither is ever silently deleted.
...
- Execution-code pin tests cover first-slot pin creation at mode `0600`, missing pin after slot one, cross-commit drift, dirty-worktree launch refusal, admission-commit mismatch and missing pin at ingestion, exclusive-create race refusal, and stale-pin preservation without silent deletion.
...
- A second adversarial pass found the matrix did not pin execution-code identity, so runs across different commits could confound the one-major-variable contract. Robert approved the bounded correction on 2026-09-04: a package-private write-once `execution-code-pin.json` created at the first slot reservation, clean-HEAD equality enforced at every later slot, `admission.code.commit` verified at ingestion, and stale pins retained as evidence. This approval repairs preparation only; it is still not the Proposal 002 Plan Gate or spend authorization.
